...
auto format_xyz(double const* p) noexcept -> QString {
    return QString("x=%1, y=%2, z=%3").arg(p[0], 0, 'f', 4).arg(p[1], 0, 'f', 4).arg(p[2], 0, 'f', 4);
}
...
auto PngOriginPickMode::supports_selection(std::optional<MouseSelection> const& selection) const noexcept
    -> bool {
    return selection.has_value() && selection->kind == pcs::AssetKind::PngMap;
}

auto PngOriginPickMode::allows_camera_interaction(Mouse const&) const noexcept -> bool { return false; }
...
auto PngOriginPickMode::on_move(Mouse& mouse, MouseEvent const& event) noexcept -> void {
    ...
    auto pixel = png_map_pixel_from_world(handle.value()->transform_view(),
        std::array<double, 3> { std::get<0>(*position), std::get<1>(*position), std::get<2>(*position) });
    if (!pixel.has_value()) {
    ...
    mouse.set_status(mode_status(QString::fromUtf8("预览 %1")
                                     .arg(format_xyz(frame.data()))));
}
